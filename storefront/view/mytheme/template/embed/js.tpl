...
var abc_token_name = '{{ constant("EMBED_TOKEN_NAME") }}';
var abc_token_value = '';
...
var init = function() {
	...
	if ( window.jQuery === undefined ) {
		...
		checkReady(
			...
			function($){
				...
				script_loader("{{ templateResource('/javascript/jquery-1.11.0.min.js') }}");
				checkReady(function($){
					jQuery = window.jQuery.noConflict(true);
					main();
				});		
	...
	function main() { 
		...
		jQuery(document).ready(function($) {
			...
			/******** Load custom modal *********/
			css_loader("{{ _this.templateResource('/stylesheet/bootstrap.embed.css') }}");
			script_loader("{{ _this.templateResource('/javascript/bootstrap.embed.js') }}");
						...
						'<div class="abcmodal-dialog abcmodal-lg">'+
							'<div class="abcmodal-content">' +
								'<div class="abcmodal-header">' +
									...
									'{% if icon %} <img src="{{ constant("HTTPS_SERVER") }}resources/{{ icon }} "/>&nbsp;{% endif %} &nbsp;{{ store_name }} ' +
									'</div><div class="abcmodal-header-menu">' +
									'<a class="abcmodal-reload" href="#" data-href="{{ account }}">{{ text_account }}</a>&nbsp;&nbsp;' +
									'|&nbsp;<a class="abcmodal-reload" href="#" data-href="{{ cart }}">{{ text_cart }}</a>&nbsp;&nbsp;' +
									'|&nbsp;<a class="abcmodal-reload" href="#" data-href="{{ checkout }}">{{ text_checkout }}</a>&nbsp;&nbsp;' +
									'</div>'+
			...
			{% else %}   
				...
				$(document).on('click', "[data-toggle='abcmodal']", function(){
					...
					{% if embed_click_action == 'same_window'%} 
						window.location = url;
					{% endif %} 
					{% if embed_click_action == 'new_window'%} 
			...
			if( !$('#abc_embed_modal').length && modal.length) {
				...
				{% if test_cookie %} 
					...
					abc_token_name = '{{ constant("EMBED_TOKEN_NAME") }}';
					abc_token_value = abc_get_cookie();
					var testcookieurl  = '{{ abc_embed_test_cookie_url }}';
					if(abc_token_value!=undefined && abc_token_value!=''){
						testcookieurl +='&{{ constant("EMBED_TOKEN_NAME") }}='+abc_token_value;
					}
